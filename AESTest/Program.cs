...
namespace AESTest
{
    class Program
    {
        ...
        {
            var encryptedData = SymmetricEncryptor.EncryptString("Test", "SOME_RANDOM_PASSWORD");

            var token = Convert.ToBase64String(encryptedData);
            encryptedData = Convert.FromBase64String(token);

            var decrypt = SymmetricEncryptor.DecryptToString(encryptedData, "SOME_RANDOM_PASSWORD");
        }
    }

    // Refer: https://tomrucki.com/posts/aes-encryption-in-csharp/ 
